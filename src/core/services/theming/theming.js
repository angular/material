...
function ThemingProvider() {
  ...
  function Theme(name) {
    ...
    self.dark = function(isDark) {
      isDark = arguments.length ? true : !!isDark;
      return self;
    };
    ...
    THEME_COLOR_TYPES.forEach(function(colorType) {
      ...
      self[colorType + 'Color'] = function setColorType(paletteName, hues) {
        var color = self.colors[colorType] = {
          name: paletteName,
          hues: angular.extend({}, hues, defaultHues)
        };
...
function generateThemes($MD_THEME_CSS) {
  ...
  function sanitizePalette(palette) {
    ...
    var darkColor = colorToRgb('rgba(0,0,0,0.87)');
    var lightColor = colorToRgb('rgb(255,255,255)');
    ...
    angular.forEach(palette, function(hueValue, hueName) {
      ...
      var rgbValue = colorToRgb(hueValue);
      if (!rgbValue) { 
