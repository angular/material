describe('<md-tooltip> component', function() {
  ...
  it('should preserve parent text', function() {
    buildTooltip(
      '<md-button>' +
      'Hello' +
      '<md-tooltip md-visible="textModel.isVisible">Tooltip</md-tooltip>' +
      '</md-button>'
    );
  ...
  it('should add an "aria-label" to the parent element', function() {
    buildTooltip(
      '<md-button>' +
  ...
  it('should not set parent to items with no pointer events',
      inject(function($window) {
        spyOn($window, 'getComputedStyle').add.callFake(function(el) {
          return {
            'pointer-events': el ? 'none' : ''
          };
        });
        ...
        buildTooltip(
          '<outer>' +
          '<inner>' +
          '<md-tooltip md-visible="testModel.isVisible">Hello</md-tooltip>' +
          '</inner>' +
          '</outer>'
        );
  ...
  it('should show after mdDelay ms', function() {
    buildTooltip(
      '<md-button>' +
      'Hello' +
    ...
    // Wait 1 below delay, nothing should still be happening
    $timeout.flush(98);
    expect($rootScope.testModel.isVisible).toBeFalsy();
  ...
  describe('show and hide', function() {
    ...
    it('should should toggle visibility on the next touch',
    ...
    it('should not show when the text is empty',  function() {
      ...
      // Trigger a change on the model, otherwise the tooltip component can't
      // detect the change.
      showTooltip(false);
      showTooltip(true);
  ...
  describe('cleanup', function() {
    it('should clean up the scope if the parent was removed from the DOM',
        function() {
          buildTooltip(
            '<md-button>' +
            '<md-tooltip md-visible="true">Tooltip</md-tooltip>' +
    ...
    it('should remove itself from the $$mdTooltipRegistry when it is destroyed',
        function() {
          buildTooltip(
            '<md-button>' +
            '<md-tooltip md-visible="true">Tooltip</md-tooltip>' +
